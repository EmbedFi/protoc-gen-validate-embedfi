...
type CustomValidatorContext func(f pgs.Field) (typeName string, rule interface{}, err error)

var Extensions = map[string]CustomValidatorContext{}

func rulesContext(f pgs.Field) (out RuleContext, err error) {

	out.Field = f
	out.CustomRules = map[string]interface{}{}
	...
	if msg := f.Type().Embed(); msg != nil {
		fullTypeName := msg.FullyQualifiedName()
		if ext, ok := Extensions[fullTypeName]; ok {
			typeName, customRule, err := ext(f)
			if err != nil {
				return out, err
			}
			if typeName != "" {
				out.Typ = typeName
				out.CustomRules[typeName] = customRule
			}
		}
